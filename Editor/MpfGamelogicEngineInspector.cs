...
namespace VisualPinball.Engine.Mpf.Unity.Editor
{
    ...
    public class MpfGamelogicEngineInspector : UnityEditor.Editor
    {
        ...
        private bool HasData =>
            _mpfEngine.RequestedSwitches.Length
                + _mpfEngine.RequestedCoils.Length
                + _mpfEngine.RequestedLamps.Length
            > 0;
        ...
        public override async void OnInspectorGUI()
        {
            if (!_tableComponent)
            {
                EditorGUILayout.HelpBox(
                    $"Cannot find table. The gamelogic engine must be applied to a table object or one of its children.",
                    MessageType.Error
                );
                return;
            }
            ...
            if (
                !string.IsNullOrEmpty(_mpfEngine.MachineFolder)
                && !_mpfEngine.MachineFolder.Contains("StreamingAssets")
            )
            {
                EditorGUILayout.HelpBox(
                    "The machine folder is not in the 'StreamingAssets' folder. It will not be included in builds.",
                    MessageType.Warning
                );
            }
            ...
            EditorGUILayout.PropertyField(
                _consoleOptionsProperty,
                new GUIContent("Console Options")
            );
            ...
            if (GUI.Button(pos, _mpfEngine.MachineFolder, EditorStyles.objectField))
            {
                ...
                var openFolder = Directory.Exists(_mpfEngine.MachineFolder)
                    ? _mpfEngine.MachineFolder
            ...
            if (GUILayout.Button("Get Machine Description"))
            {
                if (!Directory.Exists(_mpfEngine.MachineFolder))
                {
                    EditorUtility.DisplayDialog(
                        ...
                        "Gotta choose a valid machine folder first!",
                        "Okay"
                    );
                }
                else if (!Directory.Exists(Path.Combine(_mpfEngine.MachineFolder, "config")))
                {
                    EditorUtility.DisplayDialog(
                        "Mission Pinball Framework",
                        $"{_mpfEngine.MachineFolder} doesn't seem a valid machine folder. We expect a \"config\" subfolder in there!",
                        "Okay"
                    );
                }
                else
            ...
            if (GUILayout.Button("Populate Hardware"))
            {
                if (
                    EditorUtility.DisplayDialog(
                        "Mission Pinball Framework",
                        "This will clear all linked switches, coils and lamps and re-populate them. You sure you want to do that?",
                        "Yes",
                        "No"
                    )
                )
            ...
            var naStyle = new GUIStyle(GUI.skin.label) { fontStyle = FontStyle.Italic };
            ...
            {
                if (
                    _foldoutSwitches = EditorGUILayout.BeginFoldoutHeaderGroup(
                        _foldoutSwitches,
                        "Switches"
                    )
                )
                {
                    foreach (var sw in _mpfEngine.RequestedSwitches)
                    {
                        EditorGUILayout.LabelField(
                            new GUIContent(
                                $"  {sw.Id} ",
                                Icons.Switch(sw.NormallyClosed, IconSize.Small)
                            )
                        );
                    }
                ...
                if (_foldoutCoils = EditorGUILayout.BeginFoldoutHeaderGroup(_foldoutCoils, "Coils"))
                {
                    ...
                    {
                        EditorGUILayout.LabelField(
                            new GUIContent($"  {sw.Id} ", Icons.Coil(IconSize.Small))
                        );
                    }
                ...
                if (_foldoutLamps = EditorGUILayout.BeginFoldoutHeaderGroup(_foldoutLamps, "Lamps"))
                {
                    ...
                    {
                        EditorGUILayout.LabelField(
                            new GUIContent($"  {sw.Id} ", Icons.Light(IconSize.Small))
                        );
                    }
